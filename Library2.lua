...
-- Custom Logo Image
local Mercylib_img = writefile("Mercylib_logo.png", game:HttpGet(
    "https://raw.githubusercontent.com/FallensHub/FallensHub/refs/heads/main/Proyek%20Baru%207%20%5BDBA5F0C%5D.png"
))
local Mercylib_logo = getcustomasset("Mercylib_logo.png")
...
local function is_font_descriptor(value)
    return type(value) == "table" and value.__Mercylib_font == true
end
...
local RUNTIME_INSTANCE_KEY = "__Mercylib_UI_ACTIVE"
local SCREEN_GUI_NAME = "Mercylib"
...
function Mercylib.new(config)
    cleanup_previous_instance()

    local self = setmetatable({}, Mercylib)
    
    self.config = config or {}
    self.config.Name = self.config.Name or "Mercury Hub"
    ...
    self._autoConfigName = sanitize_config_name((self.config.Name or "Mercury Hub") .. "_last")
...
function Mercylib:_TrackConnection(conn)
    if conn then
...
function Mercylib:RegisterControl(flag, getter, setter)
...
function Mercylib:_RegisterRefreshJob(interval, isAliveFn, stepFn)
...
function Mercylib:_StepRefreshJobs(dt)
...
function Mercylib:_BuildAutoConfigSnapshot()
...
function Mercylib:_TryAutoSaveConfig(forceSave)
...
function Mercylib:_TryAutoLoadConfig(forceAttempt)
...
function Mercylib:_RefreshViewportMetrics()
...
function Mercylib:_InvalidateESPPreviewProjection(clearCache)
    self._espPreviewProjectionDirty = true
    if clearCache then
        self._espPreviewProjectionCache = nil
    end
...
function Mercylib:_EnsureBlurEffect()
    if self._blurEffectRef and self._blurEffectRef.Parent then
        return self._blurEffectRef
    end
    local existingBlur = lighting:FindFirstChild("MercylibBlurEffect")
...
function Mercylib:_SetBlurActive(active, instant)
...
function Mercylib:_ClearSnowflakes()
...
function Mercylib:_SpawnSnowflake()
...
function Mercylib:_SpawnRainDrop()
    if not self.snow_layer or not self.snow_layer.Parent then
        return
    end
    local width = self._cachedViewportWidth or 200
...
function Mercylib:_SpawnRainSplash(xPosition, yPosition)
    if not self.snow_layer or not self.snow_layer.Parent then
        return
    end
...
function Mercylib:_SpawnStarParticle()
    if not self.snow_layer or not self.snow_layer.Parent then
        return
    end
    local width = self._cachedViewportWidth or 200
...
function Mercylib:_SetSnowEnabled(enabled)
...
function Mercylib:_SetOverlayMode(mode)
...
function Mercylib:_SetBackgroundEffectsEnabled(enabled)
...
function Mercylib:_SetNameHidden(enabled)
...
function Mercylib:_SetTextGradientEnabled(enabled)
...
function Mercylib:_AnimateTextGradients(dt)
    if not self._uiVisualSettings.TextGradient then
        return
    end
...
function Mercylib:SetFontPreset(index)
...
function Mercylib:_RefreshAccentCore()
    local accent = self.config.AccentColor
...
function Mercylib:_ApplyOpenCloseVisuals(instant)
...
function Mercylib:SetESPProvider(providerFn)
...
function Mercylib:SetESPData(previewData)
...
function Mercylib:SetESPPreview(enabled)
...
function Mercylib:_ResolveESPPreviewWalkAnimationId(sourceCharacter)
...
function Mercylib:_StartESPPreviewWalkAnimation(sourceCharacter)
...
function Mercylib:_CreateESPPreviewFallbackCharacter()
...
function Mercylib:_DestroyESPPreviewCharacter()
    if self._espPreviewWalkTrack then
        pcall(function()
            self._espPreviewWalkTrack:Stop(0.1)
        end)
...
function Mercylib:_EnsureESPPreviewCharacter()
...
function Mercylib:_CreateESPPreviewPanel()
...
function Mercylib:_ResolveESPPreviewState()
    local state = {
        Enabled = true,
        ShowBox = false,
        ShowHealth = false,
...
function Mercylib:_ApplyESPPreviewPartVisualState(showChams, previewColor, chamsTransparency)
...
function Mercylib:_UpdateESPPreview(dt)
...
function Mercylib:SetSmoothScroll(scrollFrame, smoothSpeed)
...
function Mercylib:SetSearchFilter(rawQuery)
...
function Mercylib:SaveConfig(fileName)
...
function Mercylib:LoadConfig(fileName)
...
function Mercylib:SetToggleVisible(visible)
...
function Mercylib:SetToggleKey(keyCode)
    self.toggleKeyCode = keyCode
end

function Mercylib:BuildToggleButton()
...
function Mercylib:BuildWatermark()
    local initialText = "00:00:00 | 60 FPS"
    local pingIconWidth = 13
    local pingIconPad = 5
    local pingGap = 5
    local pingRightPad = 8
    local initialPingText = "Mercury Hub 0ms"
    local pingBoxWidth = pingIconPad + pingIconWidth + pingGap + measure_text_width(initialPingText, 11, Enum.Font.GothamSemibold) + pingRightPad
    local initialWidth = measure_text_width(initialText, 14, Enum.Font.GothamSemibold) + 45 + pingBoxWidth + 8
    ...
    create("ImageLabel", {
        Image = get_icon("atom"), BackgroundTransparency = 1,
        AnchorPoint = Vector2.new(0, 0.5),
        Position = UDim2.new(0, 8, 0.5, 0),
    ...
    self.watermark_textLabel = create("TextLabel", {
        ...
        Size = UDim2.new(1, -icon_offset - 8 - (pingBoxWidth * scale_factor), 1, 0), TextXAlignment = Enum.TextXAlignment.Left,
        Parent = self.watermark_frame
    })
    table.insert(self._gradientLabels, self.watermark_textLabel)

    -- Kotak ping terpisah, nempel di ujung kanan watermark, lebar auto-fit ke isi
    self.ping_box_frame = create("Frame", {
        Name = "PingBox",
        BackgroundColor3 = Color3.fromRGB(28, 28, 28),
        AnchorPoint = Vector2.new(1, 0.5),
        Position = UDim2.new(1, -6, 0.5, 0),
        BorderSizePixel = 0,
        Size = UDim2.new(0, pingBoxWidth * scale_factor, 0, 24 * scale_factor),
        Parent = self.watermark_frame
    })
    create("UICorner", {CornerRadius = UDim.new(0, 4), Parent = self.ping_box_frame})
    create("UIStroke", {Color = Color3.fromRGB(60, 60, 60), Thickness = 1, Parent = self.ping_box_frame})

    create("ImageLabel", {
        Image = get_icon("wifi"), BackgroundTransparency = 1,
        ImageColor3 = Color3.new(1, 1, 1),
        AnchorPoint = Vector2.new(0, 0.5),
        Position = UDim2.new(0, pingIconPad * scale_factor, 0.5, 0),
        Size = UDim2.new(0, pingIconWidth * scale_factor, 0, pingIconWidth * scale_factor),
        Parent = self.ping_box_frame
    })

    self.ping_textLabel = create("TextLabel", {
        FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold),
        TextColor3 = Color3.new(1, 1, 1), Text = initialPingText, BackgroundTransparency = 1,
        Position = UDim2.new(0, (pingIconPad + pingIconWidth + pingGap) * scale_factor, 0, 0),
        Size = UDim2.new(1, -((pingIconPad + pingIconWidth + pingGap + pingRightPad) * scale_factor), 1, 0), TextSize = 11 * scale_factor,
        TextXAlignment = Enum.TextXAlignment.Left,
        Parent = self.ping_box_frame
    })
...
function Mercylib:_UpdateWatermark(dt)
    ...
    local newText = timeNow .. " | " .. fpsCount .. " FPS"
    ...
    local pingGap = 5
    local pingRightPad = 8
    local newPingText = "Mercury Hub " .. ping_now .. "ms"
    if self.ping_textLabel then
        if self.ping_textLabel.Text ~= newPingText then
            self.ping_textLabel.Text = newPingText
        end
    end

    local pingBoxWidth = (pingIconPad + pingIconWidth + pingGap + measure_text_width(newPingText, 11, Enum.Font.GothamSemibold) + pingRightPad) * scale_factor
    if self.ping_box_frame and math.abs(pingBoxWidth - self.ping_box_frame.Size.X.Offset) > 1 then
        tween_to(self.ping_box_frame, {Size = UDim2.new(0, pingBoxWidth, 0, 24 * scale_factor)}, 0.18)
    end

    local newWidth = measure_text_width(newText, 14 * scale_factor, Enum.Font.GothamSemibold) + 45 + pingBoxWidth + 8
...
function Mercylib:_ResizeLayout(newWidth, newHeight)
...
function Mercylib:BuildMainFrame()
...
function Mercylib:BuildNotificationHolder()
...
function Mercylib:Notify(config)
    config = config or {}
    ...
    config.Icon = config.Icon or Mercylib_logo or "rbxassetid://10709768141"
...
function Mercylib:AddSection(config)
    config = config or {}
...
--]]
function Mercylib:AddConfigTab(tabObj)
    if not tabObj or type(tabObj.AddGroup) ~= "function" then
        warn("[MercylibUI] AddConfigTab: tabObj must be a valid tab returned from AddTab()")
        return
    end
...
function Mercylib:SetAccentColor(color)
...
function Mercylib.Demo()
    local lib = Mercylib.new({
        Name = "Mercylib Demo",
    ...
    misc_group:AddLabel({Name = "v1.0.0 - Mercylib Demo"})
